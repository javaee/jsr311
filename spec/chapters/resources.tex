...
\begin{description}
\item[\MatrixParam\ or \QueryParam] Either a primitive type, a class with a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument. By default, parameter values are automatically decoded; automatic decoding can be disabled using the \Encoded\ annotation. If more than one query or matrix parameter is present in the request URI then the first value (by lexical order) is selected.
\item[\PathParam] Either \code{PathSegment}, a primitive type, a class with a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument. By default, parameter values are automatically decoded; automatic decoding can be disabled using the \Encoded\ annotation.
...
\item[\CookieParam] Either \code{Cookie}, a primitive type, a class with a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument.
\item[\HeaderParam] Either a primitive type, a class with a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument.
\end{description}
