...
\begin{description}
\item[\MatrixParam, \QueryParam\ or \PathParam] Either a primitive type, a class wth a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument. By default, parameter values are automatically decoded; automatic decoding can be disabled using the \Encoded\ annotation.
...
\item[\CookieParam] Either \code{Cookie}, a primitive type, a class wth a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument.
\item[\HeaderParam] Either a primitive type, a class wth a constructor that accepts a single \code{String} argument, or a class with a static method named \code{valueOf} that accepts a single \code{String} argument.
\end{description}
...
\item Remove members for which the final matching group is neither empty nor \lq/\rq\ and the class associated with $R(T_{\mbox{class}})$ had no sub-resource methods or locators.
\end{itemize}
...
\begin{itemize}
\item Remove members that do not match $U$.
\item Remove members derived from $T_{\mbox{method}}$ (those added in step \ref{t_method_items}) for which the final matching group is neither empty nor \lq/\rq.
\end{itemize}
...
\begin{itemize}
\item If $U$ is neither empty nor equal to \lq/\rq, the method must be annotated with  a URI template that, when transformed into a regular expression using the process described in section \ref{template_to_regex}, matches $U$ with a final matching group that is either empty or equal to \lq/\rq.
