...
A URI template is a string with zero or more embedded parameters that, when values are substituted for all the parameters, conforms to the URI\cite{rfc2396} production. A parameter is represented as \lq\{\rq{\em name}\lq\}\rq\  where {\em name} is the name of the parameter. E.g.:
...
\code{HEAD} and \code{OPTIONS} requests receive additional support. On receipt of \code{HEAD} request an implementation MUST:
...
On receipt of an \code{OPTIONS} request an implementation MUST:
