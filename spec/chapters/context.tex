...
When the \Context\ annotation is applied to a resource class field, an implementation is only required to inject the applicable context into those resource class instances created by the implementation runtime. Objects returned by sub-resource locators are expected to be initialized by their creator and are not subject to resource injection by the implementation runtime.
